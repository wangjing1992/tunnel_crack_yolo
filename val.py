...
if __name__ == '__main__':
    model_path = r'E:\tunnel_crack\TCD-YOLO-best.pt'

    model = YOLO(model_path) 
    result = model.val(data= r'data/tunnel_crack.yaml',  
                        split='val', # split can be 'train', 'val', or 'test' according to your dataset
                        imgsz=512,
                        batch=16,
                        iou=0.7, 
                        device=1,
                        save_json=True, # if you need to calculate coco metrics
                        project='runs/val',
                        name='TCD_YOLO_-lite_road_crack_slim_val',
                        seed=42,
                        workers=8,  
                        hsv_h=0.025,  
                        hsv_s=0.7,  
    ...
    if model.task == 'detect': 
        ...
        print('-'*20 + ' Please refer to the following results ' + '-'*20)
        print('-'*20 + ' Please refer to the following results ' + '-'*20)
        print('-'*20 + ' Please refer to the following results ' + '-'*20)
        print('-'*20 + ' Please refer to the following results ' + '-'*20)
        print('-'*20 + ' Please refer to the following results ' + '-'*20)
        ...
        model_info_table.field_names = ["GFLOPs", "Parameters", "Preprocess Time/Image", "Inference Time/Image", "Postprocess Time/Image", "FPS (Preprocess+Inference+Postprocess)", "FPS (Inference only)", "Model File Size"]
        ...
        print('-'*20, f' Results have been saved to {result.save_dir}/paper_data.txt... ', '-'*20)
        print('-'*20, f' Results have been saved to {result.save_dir}/paper_data.txt... ', '-'*20)
        print('-'*20, f' Results have been saved to {result.save_dir}/paper_data.txt... ', '-'*20)
        print('-'*20, f' Results have been saved to {result.save_dir}/paper_data.txt... ', '-'*20)
        print('-'*20, f' Results have been saved to {result.save_dir}/paper_data.txt... ', '-'*20)