...
__all__ = ['DyHeadBlock','ADown','FocusFeature']
...
class FocusFeature(nn.Module):
    def __init__(self, inc, kernel_sizes=(5, 7, 9, 11), e=0.5) -> None:
        ...
        # ---对比组 A (标准步长卷积) ---
        # 使用 3x3 卷积，步长为 2 进行下采样，同时压缩通道到 hidc
        #self.conv3 = Conv(inc[2], hidc, 3, 2)

        # --- 对比组 B (单一最大池化) ---
        # 先用 MaxPool 下采样减半分辨率，再用 1x1 Conv 压缩通道到 hidc
        # self.conv3 = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2), Conv(inc[2], hidc, 1))
